...
export async function initVctTable(knex: Knex) {
  ...
  try {
    ...
    if (!exists) { //TODO vmarkop refactor to urn (primary) and metadata (json)
 ...
 */
export async function createVct(knex: Knex, vctName: string, vctContent: object, vctId?: UUID): Promise<number> {
...
export async function getVctByName(knex: Knex, vctName: string): Promise<TypeMetadata | undefined> {
 ...
 * @returns an array of IDs of updated rows
 */
export async function updateVctByName(knex: Knex, vctName: string, vctContent: object): Promise<number> {
  ...
  try {
    ...
    if (result === 0) {
      ...
      console.warn(`Multiple VCTs updated with name "${vctName}". This should not happen if "name" is unique.`);
    }
 ...
 */
export async function deleteVctByName(knex: Knex, vctName: string): Promise<number> {
  ...
  try {
    ...
    if (result === 0) {
      ...
      console.warn(`Multiple VCTs deleted with name "${vctName}". This should not happen if "name" is unique.`);
    }
...
export async function getAllVctMetadata(knex: Knex): Promise<TypeMetadata[]> {
  ...
  try {
    const result: Array<{ content: string }> = await knex("vct").select("content");
